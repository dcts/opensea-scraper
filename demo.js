...
(async () => {
  ...
  if (demoOffersByUrl) {
    ...
    const url =
      "https://opensea.io/collection/boredapeyachtclub?search[sortAscending]=true&search[sortBy]=PRICE&search[stringTraits][0][name]=Background&search[stringTraits][0][values][0]=Purple&search[stringTraits][1][name]=Eyes&search[stringTraits][1][values][0]=Bloodshot&search[toggles][0]=BUY_NOW";
  ...
  if (demoRankings) {
    ...
    console.log(
      `scraped ${rankings.length} collections: ${rankings
        .map((o) => o.slug)
        .join(" | ")}`
    );
  }
  ...
  if (demoOffersByScrollingByUrl) {
    ...
    const urlByScrolling =
      "https://opensea.io/collection/boredapeyachtclub?search[sortAscending]=true&search[sortBy]=PRICE&search[stringTraits][0][name]=Clothes&search[stringTraits][0][values][0]=Black%20Suit";
    const resultByScrolling = await OpenseaScraper.offersByScrollingByUrl(
      urlByScrolling,
      42,
      options
    );
